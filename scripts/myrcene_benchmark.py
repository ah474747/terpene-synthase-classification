#!/usr/bin/env python3
"""
Myrcene Binary Classification Benchmark
...
def main():
    print('🧬 MYRCENE BINARY CLASSIFICATION BENCHMARK')
    print('=' * 50)
    
    # Load data
    print('📂 Loading dataset and embeddings...')
    df = pd.read_csv('data/clean_MARTS_DB_binary_dataset.csv')
    embeddings = np.load('data/germacrene_esm2_embeddings.npy')  # Same embeddings
    
    X = embeddings
    y = df['is_myrcene'].values
    
    print(f'📊 Dataset loaded:')
    print(f'   Total sequences: {len(df)}')
    print(f'   Myrcene sequences: {df["is_myrcene"].sum()} ({df["is_myrcene"].mean()*100:.1f}%)')
    print(f'   Non-myrcene sequences: {(df["is_myrcene"] == 0).sum()}')
    print(f'   Embeddings shape: {X.shape}')
    
    # Define models (same as germacrene and pinene)
    models = {
        'XGBoost': {
            'model': Pipeline([
    ...
    # Save results
    results_path = 'results/myrcene_benchmark_results.json'
    ...
    print(f'🎯 Myrcene binary classification benchmark complete!')
