#!/usr/bin/env python3
"""
Pinene Binary Classification Benchmark
...
def main():
    print('🧬 PINENE BINARY CLASSIFICATION BENCHMARK')
    print('=' * 50)
    ...
    X = embeddings
    y = df['is_pinene'].values
    
    print(f'📊 Dataset loaded:')
    print(f'   Total sequences: {len(df)}')
    print(f'   Pinene sequences: {df["is_pinene"].sum()} ({df["is_pinene"].mean()*100:.1f}%)')
    print(f'   Non-pinene sequences: {(df["is_pinene"] == 0).sum()}')
    print(f'   Embeddings shape: {X.shape}')
    
    # Define models (same as germacrene)
    models = {
        'XGBoost': {
            'model': Pipeline([
    ...
    # Save results
    results_path = 'results/pinene_benchmark_results.json'
    ...
    print(f'🎯 Pinene binary classification benchmark complete!')
